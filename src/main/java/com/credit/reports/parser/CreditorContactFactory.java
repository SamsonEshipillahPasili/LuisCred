...
public class CreditorContactFactory {
    ...
    private void init() {
        ...
        for(int count = 1; count < rows.size(); ++count) {
            ...
            creditorContact.setCreditorName(((Element)rows.get(count)).child(0).text());
            creditorContact.setAddress(((Element)rows.get(count)).child(1).text());
            creditorContact.setPhoneNumber(((Element)rows.get(count)).child(2).text());
